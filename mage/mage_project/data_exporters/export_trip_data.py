...
def replace_zone_lookup(conn, zones_df: pd.DataFrame, engine):
    conn.execute(text("""
...
def upsert_audit(conn, audit_df: pd.DataFrame, engine):
    conn.execute(text("""
...
def delete_target_months(conn, table_name: str, df: pd.DataFrame):
    if df.empty:
        return

    month_service_rows = (
        df[['source_month', 'service_type']]
        .drop_duplicates()
        .to_dict(orient='records')
    )
...
def load_trip_df(conn, engine, df: pd.DataFrame, table_name: str):
    if df.empty:
        return 0
    ...
    delete_target_months(conn, table_name, df)
...
def export_data(data, *args, **kwargs):
    yellow_df = normalize_columns(data['yellow_trips_df'])
    green_df = normalize_columns(data['green_trips_df'])
    zones_df = normalize_columns(data['zones_df'])
    audit_df = normalize_columns(data['audit_df'])
    ...
    with engine.begin() as conn:
        ...
        yellow_inserted = load_trip_df(conn, engine, yellow_df, 'yellow_trips_raw')
        green_inserted = load_trip_df(conn, engine, green_df, 'green_trips_raw')
        replace_zone_lookup(conn, zones_df, engine)
        upsert_audit(conn, audit_df, engine)
